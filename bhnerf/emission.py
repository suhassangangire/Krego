...
def equatorial_ring(geos, mbar):
    ...
    mino_times = [krego.equatorial_lensing.r_equatorial(float(geos.spin), np.inf, float(geos.inc), mbar, float(alpha), float(beta))[1] 
...
def image_plane_dynamics(emission_0, geos, Omega, t_frames, t_injection, J=1.0, t_start_obs=None, rot_axis=[0,0,1], M=consts.sgra_mass):
    ...
    umu = krego.azimuthal_velocity_vector(geos, Omega)
    g = krego.doppler_factor(geos, umu)
    ...
    images = krego.radiative_trasfer(emission, np.array(g), np.array(geos.dtau), np.array(geos.Sigma))
    return images
